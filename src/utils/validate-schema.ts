 ...
 */
const validateSchema: (schemas: string[], request: Request) => boolean | never = (schemas, request) => {
 ...
 */
const validateQuery: (queries: string[], request: Request) => boolean | never = (queries, request) => {
    //the purpose of the unRequiredSchemas is to add any key from the request body inside it;
